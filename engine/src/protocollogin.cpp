...
void ProtocolLogin::getCharacterList(const std::string& accountName, const std::string& password)
{
	...
	for (uint8_t i = 0; i < size; i++) {
		...
		std::string ipStr = g_config.getString(ConfigManager::IP);
		unsigned ipA = 127, ipB = 0, ipC = 0, ipD = 1;
		sscanf(ipStr.c_str(), "%u.%u.%u.%u", &ipA, &ipB, &ipC, &ipD);
		output->add<uint32_t>((ipA << 24) | (ipB << 16) | (ipC << 8) | ipD);
